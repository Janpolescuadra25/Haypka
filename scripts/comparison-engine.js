...
class ComparisonEngine {
  ...
  async compare() {
    ...
    // Group by date + account (key = "YYYY-MM-DD-AccountName")
  ...
  // ── Group entries by date + account, aggregating amounts ───────
  // FIX: clone the entry before storing so mutations don't bleed.
  groupEntries(entries) {
    ...
    entries.forEach(entry => {
      ...
      if (!map.has(key)) {
        // Store a shallow clone so we never mutate the original
        map.set(key, { ...entry });
      } else {
  ...
  // ── Summary statistics ─────────────────────────────────────────
  getSummary() {
    const totalVariance = this.differences.reduce((sum, d) => sum + d.difference, 0);

    return {
      totalDifferences: this.differences.length,
      totalMatched:     this.matchedEntries.length,
      totalVariance:    Math.round(totalVariance * 100) / 100, // round to cents
...
// Make available globally (used by popup.js and dashboard.js)
